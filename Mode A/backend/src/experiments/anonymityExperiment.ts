...
function measurementConditions() {
    ...
    return {
        ...
        provingSystem: "IPA (halo2_proofs 0.3.2)",
        merkleDepth: 9,
        maxLeaves: 512
    };
}
...
async function runScenario(
    ...
    const uid = university._id.toString();

    /*
     * K6 — dung san hai phong ban truoc khi duyet.
     */
    /*
     * PHAI goi qua controller, khong goi thang service.
     *
     * `seedDefaultStaff` la service nen KHONG tu mo ket noi. Controller
     * truoc do (`createUniversityController`) da dong ket noi trong
     * `finally`, nen goi thang se chet `MongoNotConnectedError`.
     * Da gap that 2026-08-24 — cung loi da sua o runner dinh tinh.
     */
    const staff: any = await retry(
        "dung phong ban",
        () => seedDefaultStaffController(uid)
    );
     ...
     * `SystemOverloadedError` la driver TU BAO rang vong lap bi chan.
     *
     ...
     *
     * Da gap that 2026-08-25, chet o kich ban n = 100 sau khi da xong 10/12.
     */
    ...
    const pool: any = await retry(
        "tao pool",
        () => createPoolController(uid, staff.FINANCE)
    );
    ...
    for (const student of list) {
        const hb: any = await retry(
            ...
            () => issueScholarshipController(
                uid, staff.STUDENT_AFFAIRS, student.id
            )
        );
        issuedCount += 1;
    ...
    const rootApproval: any = await retry(
        "duyet root",
        () => approveRootController(pool._id.toString(), staff.STUDENT_AFFAIRS)
    );
    ...
    return {
        scenario: scenario,
        contractAddress: deployment.pool.contractAddress,
