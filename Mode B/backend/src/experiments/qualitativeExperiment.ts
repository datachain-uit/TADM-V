// Runner dinh tinh cho nhanh ON-CHAIN (ONC).
//
// PORT tu `zk-circuits-halo2-advanced/backend/src/experiments/qualitativeExperiment.ts`,
// theo chuan `zk-circuits-halo2-advanced/OFFCHAIN_REFERENCE.md`.
// Giu NGUYEN cau truc `qualitativeCriteria_2_1_3` - neu doi thi khong so
// sanh ADV vs ONC duoc nua.
//
// Khac ADV o dung hai cho, deu la dac thu on-chain:
//   1. Be mat cong khai rong hon: calldata ~3296 byte CHUA PROOF duoc
//      gui len chain (gioi han L3 trong `00_contributions.md` muc C6).
//   2. Ham `withdraw` NHAN proof => tieu chi 2 duoc cham khac, va viec
//      cham do doc tu ABI chu khong hard-code.
...
function measurementConditions() {
    ...
    return {
        ...
        k: 13,
        curve: "BN254 (Fr)",
        provingSystem: "KZG-SHPLONK (snark-verifier-sdk) - trusted setup from the constant seed ChaCha20Rng::from_seed([42u8; 32]), NOT a ceremony",
...
// Hop dong co ham nao NHAN proof lam tham so khong?
// ADV: withdrawOffChain(root, nullifier, recipient, amount) -> KHONG.
// ONC: withdraw(..., bytes proof) VA verifyAndRecord(bytes proof, ...) -> CO.
// Doc tu ABI chu khong hard-code.
//
// ⚠️ MO RONG 2026-08-25 — truoc day chi tim ten khop /^withdraw/i.
//    Sau khi tach xac minh khoi thanh toan, ham nhan proof la
//    `verifyAndRecord`, con `settle` chi nhan `bytes32 nullifier`.
//    Giu dieu kien ten cu thi tieu chi se cham SAI khi `withdraw` bi
//    go bo: bao "khong rang buoc proon-chain" trong khi that ra co.
//
//    Cau hoi dung la "co ham NAO cua hop dong nhan proof khong", khong
//    phai "ham ten withdraw co nhan proof khong".
/*
...
function withdrawBindsProofOnChain(
    ...
    return (contractInterface || []).some(
        (item: any) =>
            item.type === "function"
            && /^(withdraw|verify)/i.test(item.name || "")
            && (item.inputs || []).some(
                (input: any) =>
...
async function runQualitativeExperiment(
    ...
    /*
     * ONC ky bang tai khoan UNLOCK cua Ganache (gui `from: <dia chi>`),
     * khong ky cuc bo nhu ADV. Nen chi can dia chi, khong can khoa rieng
     * cua truong — va vi mnemonic chuan da unlock san account[0].
     */
    ...
    /*
     * Mo ket noi VO DIEU KIEN o day.
     *
     * Truoc refactor, moi service tu goi connectDatabase() nen runner
     * khong can lam gi. Nay service thuan nghiep vu, tang tren phai mo.
     * Va resetExperimentDatabaseGuarded() da dong ket noi cua no.
     */
    ...
    try {
        ...
        if (/already exists|duplicate key/i.test(error?.message || "")) {
            throw new Error(
    ...
    const pool = await createScholarshipPool(
        university.id,
        staff.FINANCE,
        undefined,
        undefined,
    ...
    const scholarship = await issueScholarship(
        university.id,
        staff.STUDENT_AFFAIRS,
        config.student.id,
     ...
     *
     * PHAI nam TRUOC `approveRoot`: root duoc duyet mot lan cho ca pool,
     * nen moi sinh vien phai co commitment trong cay TRUOC luc do. Duyet
     * root roi moi them nguoi thi ho khong chung minh duoc thanh vien.
     *
    ...
    for (const sv of extraStudents) {
        await retry(
            ...
            const hocBong = await issueScholarship(
                university.id,
                staff.STUDENT_AFFAIRS,
                sv.id,
    ...
    const web3 = getWeb3();
    const poolAddressForBalance = deployment.contractAddress;
    ...
    const usedNullifierTruoc = String(
        await getPool(deployment.contractAddress)
            .methods
    ...
    const executedRequest = await reviewWithdrawalRequest(
        request.id,
        staff.FINANCE,
        "approve"
    );
    ...
    const contract = getPool(contractAddress);
    ...
    try {
        await reviewWithdrawalRequest(request.id, staff.FINANCE, "approve");
    } catch (error: any) {
        replay = {
            rejected: true,
            transactionHash:
     ...
     * o tieu chi 4 gom `events`, `contractState` VA calldata gui sang
     * verifier. Voi n giao dich thi phai co du n calldata thi phep soi
     * moi co nghia — do dung la thu K7 sinh ra de bat.
     *
    ...
    const calldataSinhVienPhu: string[] = [];

    /*
     * V2 — xao tron TRUOC vong giai ngan. Xem chu thich cua `xaoTron`.
     */
    ...
    for (const { item, viTriNop } of thuTuGiaiNgan) {
        ...
        // Chuan hoa GIONG HET sinh vien so 0: `getStudentFromIpfs` co
        // luc tra `{note: {...}}`, co luc tra thang. Doc sai lop thi
        // bi mat thanh `undefined` va phep soi se "clean" mot cach gia
        // tao — nguy hiem hon la bao loi.
        biMatSinhVienPhu.push({
            student_id: noteCuaHo.note
                ? noteCuaHo.note.student_id
                : noteCuaHo.student_id,
            rho: noteCuaHo.note
                ? noteCuaHo.note.rho
                : noteCuaHo.rho
        });

        const yeuCau = await createWithdrawalRequest({
            cid: item.cid,
            note: noteCuaHo
        });
        ...
        // Calldata cua HO cung phai vao be mat soi, khong chi cua
        // sinh vien so 0 — do la diem manh rieng cua nhanh on-chain.
        const tepYeuCau = await WithdrawalRequest
            .findById(yeuCau.id)
            .lean();

        if (tepYeuCau?.calldata) {
            calldataSinhVienPhu.push(tepYeuCau.calldata);
        }

        /*
         * Ban ON-CHAIN cua proof ho — doc thang tu chuoi, khong phai
         * ban sao trong MongoDB. Doi xung voi sinh vien so 0.
         */
        if (tepYeuCau?.verifyRecordTxHash) {
            const giaoDichXacMinh = await web3.eth.getTransaction(
                String(tepYeuCau.verifyRecordTxHash)
            );

            if (giaoDichXacMinh?.input) {
                calldataSinhVienPhu.push(
                    String(giaoDichXacMinh.input)
                );
            }
        }
    ...
    /*
     * So cho HAI COT TRUNG NHAU — thuoc do cua kenh thu tu.
    ...
    const expectedRoot = String(rootApproval.root);
    const expectedNullifier = String(request.nullifier);

    // `createWithdrawalRequest` chi tra ve `calldataBytes`, KHONG tra
    // calldata day du. Nhung calldata moi la thu bat buoc phai soi o
    // branch on-chain (gioi han L3), nen doc thang tu document.
    const requestDocument = await WithdrawalRequest
        .findById(request.id)
        .lean();
    const verifierCalldata = requestDocument?.calldata || null;

    if (!verifierCalldata) {
        throw new Error(
            "Khong doc duoc calldata cua WithdrawalRequest "
            + request.id + ". Khong soi duoc proof gui len chain thi"
            + " tieu chi 4 se duoc cham rong hon thuc te - dung lai."
        );
    }

    /*
     * BE MAT MOI — calldata THAT cua giao dich `verifyAndRecord`.
     *
     * Tu 2026-08-25 buoc xac minh la GIAO DICH THAT chu khong con la
     * `eth.call` mo phong. Nghia la ~3 296 byte proof gio NAM VINH VIEN
     * tren chuoi, ai cung doc duoc — mot be mat cong khai that su, khong
     * phai ban sao trong MongoDB.
     *
     * `verifierCalldata` doc tu DB da chua dung nhung byte do, nhung soi
     * ban DB roi ket luan cho ban ON-CHAIN la suy dien. Doc thang tu
     * chuoi thi phep soi noi dung chinh thu ma reviewer se mo ra xem.
     */
    const verifyRecordTransaction = requestDocument?.verifyRecordTxHash
        ? await web3.eth.getTransaction(
            String(requestDocument.verifyRecordTxHash)
        )
        : null;

    if (!verifyRecordTransaction) {
        throw new Error(
            "WithdrawalRequest " + request.id + " khong co"
            + " verifyRecordTxHash. Buoc xac minh dang le phai la mot"
            + " giao dich that de lai dau vet tren chuoi. Thieu no thi"
            + " tieu chi 4 soi thieu mot be mat cong khai va se duoc cham"
            + " rong hon thuc te - dung lai."
        );
    }

    // === Be mat cong khai - RONG HON ADV ===
    // Ngoai proofOutput/ABI/event/state, ONC con phai soi:
    //   - `request.calldata`: ~3296 byte gui sang Halo2Verifier
    //   - `transaction.input`: calldata that cua giao dich settle
    //   - `verifyRecordTransactionInput`: calldata that cua giao dich
    //     verifyAndRecord — noi proof thuc su nam tren chuoi
    const publicArtifact = {
        proofOutput: {
            calldata: verifierCalldata,
            root: expectedRoot,
             ...
             * truong nen artifact trong nhu mach 3 input, du calldata da
             * mang word thu 4. Lay tu `request.recipient` — dung gia tri
             * `createWithdrawalRequest` da doi chieu voi vi dang ky.
             * Vi nhan cong khai theo thiet ke, KHONG phai bi mat.
             */
            recipient: String(request.recipient || "")
        },
        verifierCalldata,

        // K7 — calldata cua (n-1) sinh vien con lai. Mang RONG khi n = 1,
        // nen ban cu khong doi noi dung mot byte nao.
        verifierCalldataSinhVienKhac: calldataSinhVienPhu,

        transactionInput: transaction.input,
        verifyRecordTransactionInput: verifyRecordTransaction.input,
    ...
    const privateValues = {
        student_id: decryptedNote.note
            ? decryptedNote.note.student_id
            : decryptedNote.student_id,
        rho: decryptedNote.note
            ? decryptedNote.note.rho
            : decryptedNote.rho
    };
     ...
     * toan bo, nhung voi n > 1 no bo sot dung cai dang muon kiem: be mat
     * cong khai gio chua n commitment, n calldata va n giao dich rut, va
     * cau hoi that su la "co note nao trong so n bi lo khong", chu khong
     * phai "note dau tien co bi lo khong".
     *
    ...
    const fullAudit = {
        ...
        // Rieng branch on-chain: bao nhieu calldata verifier da duoc soi.
        soCalldataDaSoi: 1 + calldataSinhVienPhu.length
    };
    ...
    const calldataBytes =
        String(verifierCalldata).replace(/^0x/, "").length / 2;
    ...
    const criteria = {
        ...
        "4_private_witness": {
            ...
            evidence: {
                // Doi 25/08: them calldata THAT tren chuoi cua giao dich
                // `verifyAndRecord`. Van la MOT truong evidence, nen
                // `-tieuchi.csv` giu dung 18 dong o n = 1.
                publicSurfacesAudited: "public inputs | verifier calldata | calldata"
                    + " | on-chain verifyAndRecord calldata | events | contract state",
                ...
                /*
                 * K7 — bon dong nay CHI xuat hien khi n > 1.
                 *
                 * VI SAO PHAI CO DIEU KIEN: `buildCsvFiles` trai
                 * `evidence` thanh MOI DONG MOT TRUONG, nen them truong
                 * la `-tieuchi.csv` khong con 18 dong. Con so
                 ...
                 *
                 * n = 1 => CSV van dung 18 dong, tai lieu cu van dung.
                 * n > 1 => CSV la file MOI (`-n<so>-`), them thoai mai.
                 */
                ...(studentCount > 1
                    ? {
                        studentsAudited: fullAudit.studentsAudited,
                        studentsLeaked: fullAudit.studentsLeaked,
                        soCalldataDaSoi: fullAudit.soCalldataDaSoi,
                        leakDetails:
    ...
    const result = jsonSafe({
        ...
        // K7 — phep soi mo rong cho ca n sinh vien. Khoi RIENG, khong
        // tron vao `privacy`: draft dang trich `privacy.coverage.*`.
        ...
        amountBindingAtContract_C3: {
            ...
            verificationEvidence: "contracts/test/ShieldedPoolWithdrawGate.js (5/5) - checks the three require statements",
     ...
     * Ban n = 1 da copy sang paper/input/results/ va draft dang trich
     * thang ten truong cua no. Chay hong ma da ghi de la mat trang.
     */
    ...
    const outputPath = path.join(
        outputDirectory,
        `qualitative-onchain${nSuffix}-${timestamp}.json`
    );
    fs.writeFileSync(
        outputPath,
     ...
     * src/experiments/tinhChatChuanCsv.ts <file.json> "on-chain (ONC)"`).
     */
...
module.exports = {
    ...
    withdrawBindsProofOnChain
};

if (require.main === module) {
    require("../cli/qualitativeExperimentCli")
